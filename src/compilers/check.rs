...
pub fn package_exists(project_dir: &str, package_name: &str) {
    check_package(project_dir, package_name);
}

fn check_package(project_dir: &str, package_name: &str) {
    let args = vec!["-v", package_name];
    run_command(project_dir, "command", &args);
    return ();
}
